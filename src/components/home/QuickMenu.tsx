  ...
  return (
    <section>
          ...
          <div className="grid grid-cols-5 divide-x divide-white/15">
            {quickMenus.map((menu) => (
              <Link
                key={menu.href}
                href={menu.href}
                className="flex flex-col items-center gap-2.5 py-7 hover:bg-[#002a5c] transition-colors"
              >
                <span className="text-3xl">{menu.icon}</span>
                <span className="text-white text-sm font-medium">{menu.label}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#F5F7FA] border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center gap-x-6 gap-y-1">
          <span className="text-xs text-gray-400 font-medium shrink-0">관련기관 바로가기</span>
