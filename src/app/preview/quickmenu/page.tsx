...
const Set2 = [
  {
    label: "동문 검색",
    svg: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>,
  },
  {
    label: "회원 우대 서비스",
...
const Set3 = [
  ...
  {
    label: "올해 행사",
    svg: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>,
  },
  {
    label: "공지사항",
...
const BG = "linear-gradient(135deg, #003876 0%, #0a4f9e 40%, #1a1a2e 100%)";

function QuickRow({ items }: { items: { label: string; svg: React.ReactNode }[] }) {
  return (
    <div className="relative rounded-xl overflow-hidden" style={{ background: BG }}>
      <div className="h-44 flex items-end">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
        <div className="relative w-full pb-4 px-6">
          <div className="grid grid-cols-5 border border-white/30 divide-x divide-white/30 rounded-sm">
            {items.map((item) => (
              <div key={item.label} className="flex flex-col items-center gap-2 py-4 hover:bg-white/10 transition-all cursor-pointer text-white">
                {item.svg}
                <span className="text-[11px] font-medium text-center leading-tight px-1">{item.label}</span>
              </div>
            ))}
          </div>
        </div>
...
export default function QuickMenuPreview() {
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 font-[Pretendard,system-ui,sans-serif]">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-800">퀵메뉴 아이콘 세트 시안</h1>
          <p className="text-sm text-gray-500 mt-1">A / B / C / D / E 중 선택해주세요</p>
        </div>

        {SETS.map((set) => (
          <section key={set.id}>
            <p className="text-xs font-bold text-[#003876] uppercase tracking-widest mb-2">
              세트 {set.id} — {set.desc}
            </p>
            <QuickRow items={set.items} />
          </section>
        ))}

        {/* 세트 E — 텍스트 굵기 전용 */}
        {[
          { id: "E-1", weight: "font-medium", label: "Medium (400→500)" },
          { id: "E-2", weight: "font-semibold", label: "Semibold (600)" },
          { id: "E-3", weight: "font-bold", label: "Bold (700)" },
        ].map((v) => (
          <section key={v.id}>
            <p className="text-xs font-bold text-[#003876] uppercase tracking-widest mb-2">
              세트 {v.id} — 텍스트 전용 · {v.label}
            </p>
            <div className="relative rounded-xl overflow-hidden" style={{ background: BG }}>
              <div className="h-44 flex items-end">
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                <div className="relative w-full pb-4 px-6">
                  <div className="grid grid-cols-5 border border-white/30 divide-x divide-white/30 rounded-sm">
                    {["동문 검색", "회원 우대 서비스", "후원금 기부", "올해 행사", "공지사항"].map((label) => (
                      <div key={label} className="flex items-center justify-center py-5 hover:bg-white/10 transition-all cursor-pointer text-white">
                        <span className={`text-xs md:text-sm text-center leading-snug px-1 ${v.weight}`}>{label}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <p className="text-center text-xs text-gray-400 pb-8">원하는 세트를 알려주시면 메인에 적용합니다</p>
      </div>
    </div>
  );
}
