...
async function UsersTab() {
  ...
  const pending = (users ?? []).filter((u) => !u.is_alumni_verified && !u.is_admin);
  const approved = (users ?? []).filter((u) => u.is_alumni_verified || u.is_admin);

  const UserRow = ({ user }: { user: NonNullable<typeof users>[number] }) => (
        ...
        <form action={verifyAlumni.bind(null, user.id, !user.is_alumni_verified)}>
          <button type="submit" className={`text-xs px-3 py-1 rounded-full font-medium transition-colors ${
            user.is_alumni_verified
              ...
              : "bg-amber-50 text-amber-700 hover:bg-green-50 hover:text-green-600"
          }`}>
            {user.is_alumni_verified ? "승인됨" : "승인하기"}
          </button>
        </form>
      </td>
      <td className="py-3 px-4 text-center">
  ...
  const tableHead = (
    <thead className="bg-gray-50 border-b border-gray-200">
      <tr>
        ...
        <th className="py-3 px-4 text-center text-gray-500 font-medium w-28">승인 상태</th>
        <th className="py-3 px-4 text-center text-gray-500 font-medium w-24">권한</th>
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">회원 가입 승인</h1>
        ...
        <section>
          ...
          <div className="bg-white rounded-xl border border-red-200 overflow-x-auto">
            <table className="w-full text-sm">
              {tableHead}
              <tbody className="divide-y divide-gray-100">
                {pending.map((user) => <UserRow key={user.id} user={user} />)}
              </tbody>
            </table>
          </div>
        </section>
      ...
      <section>
        <h2 className="text-base font-bold text-gray-700 mb-3">
          전체 회원 <span className="text-gray-400 font-normal text-sm">({(users ?? []).length}명)</span>
        </h2>
